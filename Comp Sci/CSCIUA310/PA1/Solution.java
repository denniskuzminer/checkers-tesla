...
class Solution {
   ...
   public static void main(String[] args) throws Exception {
      ...
      File file = new File("C:/Users/denni/Comp Sci/CSCIUA310/PA1/2-3-range-query-small-sample-io/root.in");
      ...
      File file2 = new File("C:/Users/denni/Comp Sci/CSCIUA310/PA1/2-3-range-query-small-sample-io/root.out");
      if (!file2.exists()) {
         file2.createNewFile();
      }
      ...
      if (file.length() != 0) {
         ...
         if (n != 0) {
            for (int i = 0; i < n && ((st = br.readLine()) != null); i++) {
               ...
               int value = Integer.parseInt(st.substring(st.indexOf(" ") + 1));
         ...
         if (m != 0 && n != 0) {
            ...
            for (int i = 0; i < m && ((st = br.readLine()) != null); i++) {
               queries[i][0] = st.substring(0, st.indexOf(" "));
               queries[i][1] = st.substring(st.indexOf(" ") + 1);
               // output.write(queries[i][0] + " " + queries[i][1] + "\n");
               String x = queries[i][0];
               String y = queries[i][1];
               // output.write("----- Query " + (i + 1) + " -----\n");
               printRange(x, y, tree);
            }
         }
         output.flush();
                  ...
                  if (i == pathx.length - 1 && tree.height > 1) {
                     ...
                     if (!pathx[i + 1].getClass().getName().contains("LeafNode")) {
                        ...
                        if (p3.child0.guide.equals(guidePathX[i + 1]) && isInRange(x, y, guidePathX[i + 1])) {
                           ...
                           if (p3.child1.guide.equals(guidePathX[i + 1]) && isInRange(x, y, guidePathX[i + 1])) {
                              if (p3.child2 != null) {
                                 printAll(tree, p3.child2, pathx.length - i - 2);
                              }
                           }
